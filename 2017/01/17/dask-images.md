...
_This page includes embedded large profiles. It may look better on the [actual
site](/2017/01/17/dask-images) rather than
through syndicated pages like planet.python and it may take a while to load on
non-broadband connections (total size is around 20MB)_
...
_Electron microscopy may be generating the biggest ndarray datasets in the field - terabytes regularly. Neuroscience needs EM to see connections between neurons, because the critical features of neural synapses (connections) are below the diffraction limit of light microscopes. This type of research has been called “connectomics”. Many groups are looking at machine vision approaches to follow small neuron parts from one slice to the next._
...
up centroids in the Scikit-Image docs and came across a function that did _way_
...
This profile provides a good opportunity to talk about a scheduling _failure_;
...
is collected into 500 NumPy arrays across the cluster, each of size `(20, 1000, 1000)`.
...
Amazon's EC2) and CPU load remained high. We're _using_ our hardware.
