...
PyTorch tensors and models now serialize efficiently in Dask or in _any other
Python library_ that might want to use them in distributed systems like
...
Pickle is _not_ slow. Pickle is a protocol. _We_ implement pickle. If it's slow
then it is _our_ fault, not Pickle's.
...
_As a reminder, you can implement the pickle protocol by providing the
...
Other libraries that need to interact _definitely_ won't read the
...
There _are_ good reasons to support specialized options. Sometimes you need
